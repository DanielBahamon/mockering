class MocksController < ApplicationController
	...
	def index
		...
		@mocks = Mock.all.where(mocktype: [0...9], privated: false, reported: false, unlist: false).group(:id).order("RANDOM()").limit(20)
		@mockies =  Mock.all.where(mocktype: 0, privated: false, reported: false, unlist: false).group(:id).order("RANDOM()").limit(20)
		@mockets =  Mock.all.where(mocktype: 1, privated: false, reported: false, unlist: false).group(:id).order("RANDOM()").limit(20)
	...
	def mockets
		...
		@mocks = Mock.all.where(mocktype: [1,2], privated: false, reported: false, unlist: false).group(:id).order("RANDOM()").limit(20)
		@mockies =  Mock.all.where(mocktype: 0, privated: false, reported: false, unlist: false).group(:id).order("RANDOM()").limit(20)
		@mockets =  Mock.all.where(mocktype: 1, privated: false, reported: false, unlist: false).group(:id).order("RANDOM()").limit(20)
		@minimockers = Mocker.all.order("RANDOM()").limit(3)
	end
	...
	def mockies
		@mocks = Mock.all.where(mocktype: [0], privated: false, reported: false, unlist: false).group(:id).order("RANDOM()").limit(20)
		@mockies =  Mock.all.where(mocktype: 0, privated: false, reported: false, unlist: false).group(:id).order("RANDOM()").limit(20)
		@mockets =  Mock.all.where(mocktype: 1, privated: false, reported: false, unlist: false).group(:id).order("RANDOM()").limit(20)
		@minimockers = Mocker.all.order("RANDOM()").limit(3)
	end
