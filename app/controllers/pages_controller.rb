class PagesController < ApplicationController
	...
	def search
		...
		@mockers = Mocker.ransack(first_name_or_last_name_or_slug_cont: q).result(distinct: true).order("created_at ASC, random()").paginate(page: params[:page], per_page: 20).where(privated: false, reported: false)
