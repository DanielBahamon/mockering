  ...
  def self.all(letters)
    ...
    def self.create_from_text(mock)
      ...
      potential_matches.uniq.map do |match|
        ...
        next unless mention
        mock.update!(description: mention.markdown_string(mock.description))
    ...
    def self.create_to_review(review)
      ...
      potential_matches.uniq.map do |match|
        mention = Mention.create_from_match(match)
        next unless mention
        review.update!(comment: mention.markdown_string(review.comment))
        # You could fire an email to the user here with ActionMailer
        mocker = Mocker.find_by(slug: match.delete('@'))
    ...
    def self.create_to_answer(answer)
      potential_matches = review.comment.scan(/@\w+/i)
      potential_matches.uniq.map do |match|
        mention = Mention.create_from_match(match)
        next unless mention
        answer.update!(comment: mention.markdown_string(answer.comment))
        # You could fire an email to the user here with ActionMailer
        mocker = Mocker.find_by(slug: match.delete('@'))
        Notification.create(recipient: mocker, actor: mocker, action: "mentioned", notifiable: answer)
        mention
      end.compact
    end
