class Mock < ApplicationRecord
	...
	# For music
	has_attached_file :music, proccessors: [:transcoder]
	...
	before_post_process :movie do
		...
		if self.duration == nil && self.picture_file_name == nil && self.music_file_name == nil
	...
	# def skip_for_picture
	# 	! %w(/\Aimage\/.*\z/).include?(asset_content_type)
	# end
